...
#' \insertCite{Diaz2024;textual}{rcaiman.registry}. Next explanation might serve mostly
...
#'
#' *The following code predates the current recommended workflow. For
#' reproducibility and auditability, create the registry entry first, define its
#' identity signatures and geometry specification, and reference those objects
#' explicitly in the calibration code instead of hard-code specifications in the
#' script.*
#'
#' ````
#' zenith_col_row <- c(1500, 997)
#' diameter <- 947*2
#' z <- zenith_image(diameter, c(0.689, 0.0131, -0.0295))
#' a <- azimuth_image(z)
#'
#' .read_raw <- function(path_to_raw_file) {
#'   r <- read_caim_raw(path_to_raw_file)
#'   r <- crop_caim(r$B, zenith_col_row - diameter/2, diameter, diameter)
...
#'
#' ref <- l[[1]]
#' rings <- ring_segmentation(zenith_image(ncol(ref), equidistant_coef), 3)
...
extract_radiometry <- function(l, size_px = NULL) {
  ...
  r <- max(terra::rast(l))
  z <- zenith_image(ncol(r), equidistant_coef)
  a <- azimuth_image(z)
