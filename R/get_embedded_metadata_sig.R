#' Extract embedded metadata signature
#'
#' @description
#' Retrieve a embedded metadata signature within a registry entry.
#'
...
get_embedded_metadata_sig <- function(registry_entry, embedded_metadata_sig) {


  .check_registry_entry(registry_entry)
  .check_vector(embedded_metadata_sig, "character", 1)

  if (!embedded_metadata_sig %in% names(registry_entry) ||
      !.is_embedded_metadata_sig(registry_entry[[embedded_metadata_sig]])) {
    stop(
      "Embedded metadata signature '",
      embedded_metadata_sig,
      "' not found in this registry entry"
    )
  }

  registry_entry[[embedded_metadata_sig]]
}
