...
#'
#' @description
#' Construct a registry entry describing a imaging system and its
#' physical components (camera body, device, lens, and/or auxiliary lens).
#'
#' @details
#' A registry entry represents a *physical acquisition system*
...
#'
#' @return An object of class [hs_registry_entry].
#'
#' @seealso [add_file_sig()], [add_embedded_metadata_sig()], [add_geometry_spec()],
#'   [add_radiometry_spec()]
#'
#' @export
#'
...
new_registry_entry <- function(
  ...
  if (!(has_device || case_compact || case_reflex)) {
    stop(
      paste(
        "Invalid hemispherical system configuration.",
